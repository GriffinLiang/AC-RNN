...
for jj = 1:size(sequence_label{2}, 1)
    pos_num = sum(sequence_label{2}(jj, :) == 1);
    neg_num = sum(sequence_label{2}(jj, :) == 0);
    weight{2}(jj, sequence_label{2}(jj, :)==1) = (pos_num + neg_num)/(2*pos_num);
    weight{2}(jj, sequence_label{2}(jj, :)==0) = (pos_num + neg_num)/(2*neg_num);
end
options.maxIter = 400 ;
